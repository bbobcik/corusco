...
selectedCustomerName.setValue("Grace Hopper", StandardChangeOrigin.USER);
subscription.close();
\end{lstlisting}
...
uses \api{StandardChangeOrigin.MODEL}; callers may supply another origin when the
difference matters.
...
ValueChangeListener<CustomerRow> listener = event -> {
    if (event.origin() == StandardChangeOrigin.USER) {
...
with \api{CustomChangeOrigin.of}.
...
selectedCustomer.setValue(rowFromTable, StandardChangeOrigin.USER);
selectedCustomer.setValue(rowFromInitialLoad, StandardChangeOrigin.MODEL);
\end{lstlisting}
...
\api{StandardChangeOrigin.USER}. Presenter-originated changes retain their model origin
...
selectedCustomerId.setValue(new CustomerId(42), StandardChangeOrigin.USER);
\end{lstlisting}
...
dirty.setValue(true, StandardChangeOrigin.USER);
...
\api{CustomChangeOrigin.of} should be named as a stable fact, not as a sentence that
...
doubled.close();
source.setValue(2, StandardChangeOrigin.MODEL);
...
selected.setValue(rowFromUserClick, StandardChangeOrigin.USER);
selected.setValue(rowFromRestore, StandardChangeOrigin.MODEL);

assertThat(origins).containsExactly(StandardChangeOrigin.USER, StandardChangeOrigin.MODEL);
\end{lstlisting}
...
void tableSelectionChanged() {
    ...
    selectedCustomerId.setValue(id, StandardChangeOrigin.USER);
}
...
void applyFilter() {
    committedFilter.setValue(draftFilter.value(), StandardChangeOrigin.USER);
}
\end{lstlisting}
...
void commitPreferences() {
    preferences.theme().setValue(draftTheme.value(), StandardChangeOrigin.USER);
    preferences.pageSize().setValue(draftPageSize.value(), StandardChangeOrigin.USER);
}
\end{lstlisting}
...
\item Add a test that records \api{StandardChangeOrigin.USER} and
\api{StandardChangeOrigin.MODEL} transitions separately.
