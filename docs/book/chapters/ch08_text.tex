...
The screenshots in this chapter are produced by \api{BookVisualFigures} through \api{BookFigureGenerator}. Figures~\ref{fig:text-document-model}, \ref{fig:text-validation-state}, and~\ref{fig:text-mutation-pipeline} are generated Swing panels. The relevant runnable binding examples are \api{SwingBindingExample} and \api{BehaviorExample}; the screenshot harness is \screenshotHarnessExample. Class names are cited deliberately so the PDF does not contain fragile wrapped paths.
\end{examplebox}
