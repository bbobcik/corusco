...
Create a tiny presenter object with fields for state and methods for events.
...
view row, convert it to a model row before presenter code asks for the selected
object. If presenter code wants to select an object by id, resolve the id in
...
severity? Those decisions can be tested as model or presenter logic. The
...
Global singletons have a similar risk. A global help service, resource service, or diagnostic registry may be appropriate. A global "current customer panel" is not. Prefer passing services through presenters, behavior contexts, or application shells.
...
into a presenter or model. Third introduce Corusco values, commands, problems,
...
The same story should be visible in code. A presenter or model names the facts.
...
owner they need. Some findings need plain refactoring: a presenter method, a
