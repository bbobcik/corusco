...
\api{BookVisualFigures} contains the source for Figures~\ref{fig:painting-dirty-regions}, \ref{fig:painting-opacity}, \ref{fig:painting-composite-src-over}, \ref{fig:painting-composite-group-opacity}, \ref{fig:painting-composite-clear-mask}, and~\ref{fig:painting-composite-rule-comparison}. The screenshots are generated by \api{BookFigureGenerator} through \screenshotHarnessExample{}, so the book uses rendered Swing examples rather than copied illustrations.
\end{examplebox}
...
The third test layer is visual. The screenshot example fixes data, size, Look-and-Feel, and state, then captures the timeline. This test will not prove that every pixel is right on every platform, but it will catch accidental theme colors, missing opacity, stale overlay text, and layout drift in the book figures. Visual examples should be deterministic enough to serve as documentation and smoke tests.
