...
\api{ChangeOrigin.USER}. A presenter can then distinguish a user's keystroke
...
\api{ChangeOrigin.USER}, observes model raw-text changes, and closes both
...
\api{ChangeOrigin.USER}. Model events update the button while a guard prevents
echo writes.
...
\api{ChangeOrigin.USER}; model changes update the checkbox under a feedback
...
select-all-on-focus. For a checkbox it installs selected-state binding. The
generated plan is valuable precisely because it is boring. A reader can open it
and see the same calls a careful developer would have written by hand.
...
Generated \api{@SwingForm} companions can produce behavior plans for a view
...
nameField.getDocument().addDocumentListener(new DocumentListener() {
    ...
    public void insertUpdate(DocumentEvent event) {
        form.name.setRawText(nameField.getText(), ChangeOrigin.USER);
    }
    @Override
