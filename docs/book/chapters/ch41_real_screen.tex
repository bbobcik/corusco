...
\section{Screen-Presenter State and Command Vocabulary}

The screen presenter, in the sense defined in Chapter~\ref{ch:models-views-presenters}, owns screen state that is not purely visual: observable row
...
The screen presenter is the place where screen facts become workflow. It knows that a
...
\begin{lstlisting}[caption={Screen-presenter state sketch.}]
...
Commands are part of screen-presenter state because they express operations in terms
...
The screen presenter supplies runtime availability. This distinction lets a toolbar
...
description. The screen presenter is the place to derive that reason from form,
permission, and task state.

Screen-presenter state should be testable without Swing where possible. A core test
...
boolean forces the view to invent text it cannot justify. Screen-presenter state can
...
screen presenter prevents the view from turning every "go back" gesture into a local
button listener.
...
which screen-presenter values receive selected coordinates and row values. The
...
selected state. Screen-presenter logic reacts to selected state. Detail loading uses
tasks or master-detail values as appropriate.

\begin{lstlisting}[caption={Selection reaction in screen-presenter state.}]
...
this accidentally. The screen presenter should compare durable row ids, not view
...
present the cell. Do not put screen-presenter state into renderer components.
...
The view owns components and layout. The screen presenter owns when a form is loaded,
...
screen-presenter state. Generate and inspect companions. Create the MigLayout view.
...
inspected before the view is finished. After screen-presenter state exists, command
...
into screen-presenter values. Commands that depend on selection update. The detail
...
there. The screen presenter should not build MigLayout panels. It should not subclass
...
install a table model from a descriptor, bind selection into screen-presenter values,
