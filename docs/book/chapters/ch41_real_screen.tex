...
constructed on the EDT, tested, and captured into the PDF. The chapter will
...
visible. The view layout should be visible. The screenshot harness should be
visible. If understanding the screen requires following anonymous listeners
through component construction order, the code may work but the chapter's
...
use the same resource map so the PDF shows the text that the example actually
resolves.
...
Screenshot generation should not be hidden in a manual recipe. The book uses
figure files under the TeX tree, but their source should remain in companion
examples or figure generators. A developer should be able to find the Java
source for a figure, run the harness, and understand which screen state is
being captured. The caption and surrounding text should then mention the short
example name, not a long filesystem path that wraps in the PDF.
...
The seventh failure pattern is screenshot drift. The book or application has a
figure, but the figure is no longer generated from the example. The screen in
the PDF shows different labels, spacing, or state from the current code. The
...
disposed components.
\item Generate a screenshot and include it in a local PDF build.
\end{enumerate}
\end{exercisebox}
