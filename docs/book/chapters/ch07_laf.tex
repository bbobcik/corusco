...
The companion screenshots install \flatlaf{} first and use \miglayout{} for ordinary panels, so the figures show production-shaped screens rather than isolated toy controls. They cover ordinary component states, dense screen metrics, and the state-to-delegate ownership chain.
\end{examplebox}
...
\begin{coruscobox}{Named Visual State}
...
Screenshot names should be treated as part of the example contract. A useful name says which state is being rendered without exposing local file layout or transient tooling details. A long absolute path inside a report or figure box is not documentation; it is an implementation leak.
