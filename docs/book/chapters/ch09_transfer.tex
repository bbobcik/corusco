...
The transfer screenshots are produced by \api{BookVisualFigures} through \api{BookFigureGenerator}. The screenshot harness is \screenshotHarnessExample. The figures deliberately cite compact class names rather than full paths, so the PDF documents the example contract without exposing repository layout. The three transfer figures capture drop feedback, import progress, and the flavor-to-workflow boundary.
\end{examplebox}
...
\begin{detailbox}{canImport should be cheap and honest}
