...
\begin{examplebox}{java2d figures}
\api{BookVisualFigures} contains the source for Figures~\ref{fig:java2d-vector-primitives} and~\ref{fig:java2d-scale-grid}. The screenshots are produced by \api{BookFigureGenerator} through \screenshotHarnessExample{}, which fixes the Look-and-Feel, window size, and output path for the book build.
\end{examplebox}
...
Screenshot generation is a special profile, not merely a file output. A book figure should render with deterministic data, deterministic size, deterministic theme, and no wall-clock animation. It may still use the same geometry and painters as the live UI. This is valuable because screenshot examples then exercise real rendering code. It is dangerous only when the screenshot path silently changes geometry to make the picture pretty. A figure should be curated, but it should not be a different application.
