...
Primary APIs & \api{FormDialog}, \api{FormDialogShell}, \api{FormDialogLifecycle}, \api{FormDialogValidationBinding}, \api{FormDialogKeyboardBinding}, \api{ProblemFocusResolver}, \api{SwingEditors}, \api{DirtyState}, and \api{CancelConfirmation}.\\
...
Companion examples & \dialogExample, \dialogValidationExample, \dialogActiveEditorExample, \dialogDirtyCancelExample, \dialogLifecycleExample, and \dialogKeyboardExample.\\
\end{chaptermap}
...
cancel, or fail to accept the current input. It should not half-succeed by
closing the window while leaving the caller to guess what happened.
...
controller. It owns OK, Apply, and Cancel semantics for a form model. It does
...
reached a terminal closed state. The OK, Apply, and Cancel commands become
disabled. The accepted \api{DialogResult} is final. The shell may then dispose
the native peer, and the caller may continue with the result. This one-shot
rule makes tests and cleanup much simpler. There is no second OK after a
successful OK, and there is no hidden branch in which the controller closes but
the result remains mutable.
...
\api{acceptCurrentValues} after a successful Apply. A program that chooses a
different policy should not hide it in a listener; it should name it and test
it.
...
The controller also exposes OK, Apply, and Cancel as mutable commands. Commands
...
and generated companions stable identities. The enabled state follows
\api{refreshCommandState}: OK and Apply are enabled only while the controller
is open and the form is committable; Cancel is enabled while the controller is
open. If the form model does not expose observable command-state changes, call
\api{refreshCommandState} after validation or parse state changes. This is a
small explicit call, but it is better than hidden listeners that revalidate the
dialog at surprising times.
...
remaining open. Cancel may leave it open if dirty confirmation refuses, or move
it to cancelled and closed. Close always moves an open controller to cancelled
and closed without asking confirmation. Every transition refreshes commands so
the visible controls do not advertise actions that the controller will ignore.
The state machine is small enough to test exhaustively, and that is one of the
main reasons to keep it out of anonymous listeners.
...
The book's example references are intentionally short. The printed chapter
should say \dialogExample{} or \dialogValidationExample{}, not a full package
path that wraps across the page. The repository structure can move; the class
name remains a readable entry point. The same convention should be used in new
chapters: name the example by the didactic program, and let the examples index
or build system point to its physical location.
...
\begin{examplebox}{dialogs}
\dialogExample{} demonstrates base OK, Apply, and Cancel controller semantics.
...
Apply then Cancel loses applied work & Is the dirty baseline moved after successful Apply according to policy?\\
