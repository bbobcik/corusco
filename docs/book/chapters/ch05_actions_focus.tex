...
\section{Commands Before Command Surfaces}
...
\caption{A routed command resolves the active context before execution, then publishes command state back to every command surface. Focus owner, semantic owner, command id, and visual surface are related but distinct facts.}
...
execute the handler if appropriate, and update all command surfaces from the same
...
\textbf{Action properties describe the command.} Standard properties include \api{Name}, \api{SMALL\_ICON}, \api{SHORT\_DESCRIPTION}, \api{ACCELERATOR\_KEY}, \api{MNEMONIC\_KEY}, and \api{SELECTED\_KEY}. Command surfaces listen to property changes and update themselves. Custom properties can be useful, but they should be documented because only surfaces that know the property will honor it.

A subtle rule follows: changing an action's name or icon changes it for all command surfaces sharing the action. If a toolbar needs icon-only presentation and a menu needs text, configure the surface. Do not mutate the shared action to satisfy one surface. The action's properties should describe the command; the command surface decides how much of that description to show.
...
\termdef{Command presenter}{a visual or input surface that invokes a command} includes buttons, menu items, popup items, toolbar buttons, key bindings, default buttons, and sometimes accessibility actions. Chapter~\ref{ch:models-views-presenters} distinguishes this term from a screen presenter, which owns workflow and screen state. Command presenters may be created and destroyed with views. Commands may live for the screen, document, application, or focused context. Mixing these lifetimes is a common source of stale enabled state and memory leaks.
...
and active context. The second question points to one command surface. The same
...
Command presenters have their own lifetimes and density rules. A toolbar may be rebuilt
...
without creating buttons at all. If the command is stored inside one command presenter,
all other command presenters become second-class copies. If the command exists apart
from command presenters, each surface can be created and destroyed without losing the
operation's state.
...
default button is command-presenter state. A long-running Save operation may introduce
...
shortcut still invokes a command after its command presenter disappeared.
...
Application/\allowbreak global & What command remains valid across windows? & Forgetting that each top-level window has its own focus and modality.\\
\bottomrule
\end{tabularx}
\end{center}
...
Corusco command descriptors make command identity, resources, help, enablement, selected state, shortcuts, and generated metadata explicit outside concrete Swing command presenters. Swing's \api{Action} remains the adapter to buttons, menus, and key bindings. Corusco's value is that the command contract becomes inspectable and testable before a toolbar happens to exist.

\begin{coruscobox}{Commands as Data}
Corusco treats command identity, resource keys, help topics, shortcuts, enablement, selected state, and lifecycle as named model facts. The Swing \api{Action} is then a command-presenter adapter, not the only place where command meaning lives.
\end{coruscobox}
...
Corusco also helps with command context. A screen can expose selected domain ids, active editor state, dirty state, problem state, busy state, and permission state as observable values. Command enablement can derive from those values. The same derivation can feed disabled reasons, command-surface enabled flags, and tests. This is more maintainable than having each \api{Action} search the component tree.
...
Migration can be incremental. Start by naming important commands and replacing duplicate listeners with shared actions. Next, move labels, shortcuts, and help topics into stable resource-backed descriptors. Then derive enablement from observable state. Finally, let generated adapters install command surfaces and bindings. Each step removes a class of drift without requiring a full rewrite.
...
ask a screen to list installed commands, shortcuts, command presenters, enabled states,
...
derived values. Finally, generate adapters for routine command presenters. Each step is
...
Icons are also command resources, but icon size is often command-surface-specific. A menu item may use a small icon or none. A toolbar may use a larger icon. A command palette may use a symbolic icon plus text. The command can name the icon resource; the command surface chooses size and layout. This keeps the command semantic and the visual surface flexible.

Selected state belongs to toggle commands. A toggle command should have one boolean or enum state that command presenters observe. Some toggles are binary, such as "show inactive." Others are mutually exclusive modes, such as select, pan, zoom, and edit. A group of mode commands should be backed by one mode value, not by buttons that deselect one another through listener gossip.
...
Logging and telemetry should use command ids. "customer.save invoked" is stable; "button Save clicked" is a command-presenter accident. If users invoke Save by shortcut, menu, or automation, the command id stays the same. Command-surface-specific logging can be useful for usability research, but it should not replace semantic command logging.
...
The reader should notice the repeated shape: command facts first, Swing command presenters second. Button construction, menu construction, key binding installation, focus traversal, validation, undo, help, and tests all become easier when the command is a named object with modeled state. This is the command equivalent of the table chapter's stable row identity.

A command registry should be inspectable at runtime. During development, it is useful to open a small diagnostic view that lists command id, text resource, enabled state, selected state, shortcut, shortcut scope, disabled reason, help topic, and command presenters. This is the command equivalent of a repaint manager log. It turns invisible routing into data that can be reviewed.
...
When introducing Corusco to an existing Swing application, the registry diagnostic is a useful migration tool. First list existing actions and listeners. Then merge duplicate command presenters under stable command ids. Then move resources and shortcuts into descriptors. Then derive enablement from observable state. Each migration step can be measured by the diagnostic view: fewer duplicate actions, fewer unknown shortcuts, fewer command-presenter-only labels.
...
The command that lies about enablement is the standard failure. A Delete button is disabled when no row is selected, but the Delete menu item remains enabled because it has a separate listener and separate predicate. The keyboard shortcut still works because it bypasses both. This is not a button bug. It is a command architecture bug. One command should own semantic enabled state, and command presenters should reflect it.
...
the whole pattern: stable ids, several command presenters, context routing, active
...
\textbf{Checklist for command and focus architecture.} Use \api{Action} or command descriptors for reusable operations. Share one command across menu, toolbar, popup, and key binding command presenters. Separate command state from visual command-presenter state. Use platform menu shortcut masks. Put window-level key bindings in one place. Avoid broad replacement of built-in input maps. Log focus before debugging key bindings. Use \api{InputVerifier} only for fast local checks. Preserve labels, mnemonics, focus order, and keyboard alternatives. Test actions without requiring mouse automation.
