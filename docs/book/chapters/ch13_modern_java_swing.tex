...
\modernJavaExample{} supplies the small sealed-state screen for Figure~\ref{fig:modern-java-state-screens}. The task-boundary figure later in the chapter uses the same compact, deterministic style so the focus stays on state and task ownership rather than on source-tree layout.
\end{examplebox}
...
public record CustomerRow(
        ...
        @Column(width = 220) String displayName,
        @Column(width = 120) CustomerStatus status,
        @Column(width = 120) LocalDate lastOrderDate) {
}
\end{lstlisting}
...
Processor diagnostics should point at the source declaration that caused the problem. If \api{@CoruscoForm} supports only records, the diagnostic should point at the non-record type. If a column width is invalid, the diagnostic should point at the annotated component. If a resource prefix is blank, the diagnostic should say so. Good compile-time diagnostics are part of the UI library's developer experience.
...
\item Annotate a record with \api{@CoruscoTable}, inspect the generated columns, and find one generated resource key.
