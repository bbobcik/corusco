...
Generated names need special attention because names become the bridge between documentation, IDE navigation, and tests. If \api{CustomerEditFormModel} is the generated model and \api{CustomerEditSwingBindings} is the generated binding companion, the names should be stable enough for the book to mention them. A naming change is sometimes justified, but it should break a generation test and force the documentation to be updated deliberately.
...
Screenshots are essential for this book because Swing is visual and because visual chapters should show what they discuss. But screenshots are a poor substitute for model tests. A screenshot can prove that a figure is not blank, that a form is not clipped, that a table state is visible, that an overlay covers the intended region, or that FlatLaf and MigLayout produce a coherent screen. It cannot prove every validation rule, command rule, or generated descriptor by itself.

\termdef{Screenshot smoke test}{a deterministic rendering test that verifies a screen state can be built, laid out, painted, and reviewed as pixels} is the right default for book figures. It should use seeded data, a fixed size, a known Look-and-Feel, stable fonts where practical, and explicit output paths. It should not require a human to open the application and manually arrange a window before the PDF can be built.

\api{BookScreenshotHarness} is the companion harness for this role. It installs the example Look-and-Feel, lays out component trees, paints them into images, and writes PNG files used by the LaTeX build. It is not a general visual-diff framework. It is a disciplined way to ensure that figures are generated from executable Swing examples rather than from copied mockups.
...
The important facts are visible: output name, deterministic size, and a factory for a real Swing component. A test can construct the same component and assert that the screenshot file is produced. A reviewer can inspect the PNG. The LaTeX chapter can include it. The code path stays connected from example to figure to PDF.
...
Screenshot tests often reveal documentation problems. A caption may look too much like body text. A figure may be too small to read. A long example path may wrap in an ugly way. A screenshot may contradict the prose because the code changed. These are real defects. Documentation is a user interface for ideas, and screenshots are part of that interface. The book build should therefore treat screenshot generation as a first-class acceptance step, not an afterthought.

\begin{migrationbox}{from manual figure capture to generated figures}
Manual screenshots drift. A Corusco book example should expose a small component factory, the screenshot harness should render it under FlatLaf at a fixed size, and the chapter should include the generated PNG. Tests can then compile the example and smoke-test the harness before the PDF relies on the image.
\end{migrationbox}
...
The strongest failure messages include the layer. A core model failure can say "form problem set." A Swing binding failure can say "component-to-model text binding." A generated-source failure can say "generated CustomerEditFormModel." A screenshot failure can say "book figure testing-component-keys.png was blank" or "expected output size 760 by 360." Layer words reduce the time between seeing a failure and opening the right file.
...
The book build adds another CI-like constraint. Figures should be generated from examples, and the PDF should include them consistently. The TeX build should not depend on a developer's manually opened application window. Dockerized TeX can compile the book, but it cannot rescue a missing PNG or a screenshot that was never generated. Example tests and screenshot harness tests protect this path before LaTeX runs.
...
Resource cleanup should be explicit when examples generate screenshots. If a screenshot harness installs a Look-and-Feel, creates images, writes files, or temporarily changes UI defaults, the harness should confine those effects. A book figure generator that leaves global state changed can affect later figures in subtle ways. This is the documentation version of the same lifecycle rule.
...
The seventh rung is screenshot smoke. Render the normal workspace and busy-detail workspace under FlatLaf at fixed sizes. Assert the files exist and are nonblank, then review the images in the book. The screenshots prove that the assembled screen looks like the prose says it looks. They are late in the ladder because they are broad and visual.
...
The final review habit is to read the tests after changing the book. If a chapter cites \api{SwingMvpTesterTableStateExample}, there should be a test for that example. If a chapter includes \testingHarnessFigure{}, the screenshot harness should generate it. If a chapter says generated examples compile through the processor, a generated-source test should exist. The book's acceptance criteria and the code's acceptance criteria should reinforce each other; otherwise the PDF can become more confident than the source deserves.
...
\item Use screenshot smoke tests for visual coherence, book figures, layout, theme, density, overlays, and nonblank rendering.
