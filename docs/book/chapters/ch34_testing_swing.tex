...
\termdef{Testing split}{the discipline of testing a rule at the lowest layer that owns it} is the central idea of this chapter. A form validation rule belongs in the form model. A generated descriptor belongs in generated source. A command enablement rule belongs in the command or screen-presenter model, using the vocabulary from Chapter~\ref{ch:models-views-presenters}. A text binding belongs at the Swing boundary because a \api{JTextField} document is involved. A table-selection conversion belongs at the Swing table boundary because view rows and model rows are Swing concepts. A screenshot belongs at the visual boundary because pixels, density, theme, and layout are the evidence.
...
form.creditLimit().setRawText("-1", StandardChangeOrigin.USER);
...
form.name().setRawText("Ada", StandardChangeOrigin.USER);
...
This is the pattern that later appears in \api{SwingMvpTesterProblemExample}. The Swing test can type into a field and assert that the screen presenter's problem set contains \api{REQUIRED} for the \api{NAME} field key. It should not search a tooltip for "Name is required" unless the tooltip text is the rule being tested.
...
\termdef{EDT test}{a test whose Swing construction, mutation, and query work is executed on the Event Dispatch Thread} is not the same as a test that simply calls \api{SwingUtilities.invokeAndWait} around one assertion. The whole fixture has to be honest about thread ownership. View factories should run on the EDT. Screen-presenter construction should run on the EDT if it touches the view. Component lookup should run on the EDT. Command invocation should run through the same thread expectations as production code.

\api{SwingMvpTester} is the small Corusco test fixture for this boundary. It constructs a view and, optionally, a screen presenter. It provides \api{runOnEdt} for mutations and \api{queryOnEdt} for reads. It can locate components by typed keys, drive common component inputs, execute commands, assert problems, inspect behavior scopes, and verify table state. It does not make Swing free. It makes the Swing boundary explicit.
...
The factory form matters. A test that constructs the view before creating the tester has already made a threading decision. A test that creates a screen presenter outside the tester may accidentally mutate the view off the EDT in the presenter's constructor. The tester factory centralizes this boundary. A large application may have its own fixture, but it should preserve the same visibility: where is the view created, where is the screen presenter created, and where are Swing components touched?
...
tester.runOnEdt((view, presenter) -> {
    ...
    model.setRawText("Ada", StandardChangeOrigin.MODEL);
    assertThat(field.getText()).isEqualTo("Ada");

    presenter.close();
    model.setRawText("Late", StandardChangeOrigin.MODEL);
    assertThat(field.getText()).isEqualTo("Ada");
});
\end{lstlisting}
...
Compile-time generation changes what must be tested. If a chapter or API says that \api{@CoruscoForm} produces a form model, component keys, binding companions, and validation metadata, then a test should compile annotated source and inspect the generated output. It should not rely on a handwritten imitation of the generated class. The processor is the contract.
...
The generated-source layer also guards examples. Book examples that demonstrate \api{@CoruscoTable}, \api{@UiAction}, validation annotations, help annotations, or generated component keys should compile through the processor. A screenshot produced from a generated view is stronger when the generated companions were produced in the same build. The reader can then trust that the listing is not decorative pseudo-code.
...
scope.close();
statusText.setValue("late", StandardChangeOrigin.MODEL);
...
Table state & \api{TableStateController.\allowbreak captureState} by table and column ids. & Screenshot showing restored order, width, visibility, and sort.\\
