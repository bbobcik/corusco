...
Primary APIs & \api{ObservableList}, \api{ObservableArrayList}, \api{ListChange}, \api{ListChangeSet}, \api{FilteredList}, \api{SortedList}, \api{MappedList}, \api{LoadableList}, \api{EdtObservableList}, and \api{GlazedObservableList}.\\
...
publishing partial intermediate states.
...
DerivedValue<Boolean> canExport = DerivedValue.of(
        () -> !visibleRows.snapshot().isEmpty()
                && searchState.value() instanceof SearchState.Loaded,
        searchState);
\end{lstlisting}

The listing is intentionally imperfect: \api{canExport} reads a snapshot but
does not observe the row list. In real code, export enablement might be updated
by a list-size value, a table descriptor, or a command model that observes both
the list and the load state. The imperfection is a teaching point. Observable
lists are collections, not scalar values. When scalar state depends on
collection size, decide how that scalar will be observed. Some screens derive
size by subscribing to the list and updating a \api{rowCount} value. Others
derive command enablement inside the table companion. The policy should be
visible.
...
void afterRefresh() {
    ...
    boolean stillPresent = rows.snapshot().stream()
...
assert tags.snapshot().contains(new TagChoice("urgent"));
\end{lstlisting}
