...
Primary APIs & \api{@UiAction}, \api{@CoruscoForm}, \api{CommandSet}, \api{BehaviorScope}, \api{CommandBehaviors}, \api{SwingMvpTester}, generated form companions, generated action companions, table descriptors, and table-state controllers.\\
...
final class CustomerEditorPresenter implements AutoCloseable {
    ...
    void save() {
        ...
        saveBusy.setValue(true, StandardChangeOrigin.SYSTEM);
        service.save(form.result());
        saveBusy.setValue(false, StandardChangeOrigin.SYSTEM);
    }
    ...
    private void loadDetail(CustomerId id, CustomerService service) {
        ...
        service.loadAsync(id, loaded -> {
            if (generation.isCurrent(token)) {
                detail.setValue(loaded, StandardChangeOrigin.SYSTEM);
...
Next declare stable generated sources where they have clear payoff. If the screen has a form with more than a trivial amount of validation, declare a \api{@CoruscoForm} source. If the screen has repeated command surfaces, annotate screen-presenter methods with \api{@UiAction}. If the screen has a table whose columns will be tested, persisted, exported, or localized, define a descriptor. These declarations create the shared handles that later code should consume.
...
Declarations & \api{@CoruscoForm} records, \api{@UiAction} methods, descriptors.\\
...
void saveCommandUsesGeneratedIdentity() {
    ...
    form.creditLimit().setRawText("100.00", StandardChangeOrigin.USER);
