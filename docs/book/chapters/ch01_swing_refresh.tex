...
larger application may split them into view, screen presenter, generated binding
...
tell which view row is selected; screen-presenter code should usually convert that to
...
Corusco commands make action identity and resources toolkit-neutral. A Corusco command can be adapted to a Swing \api{Action}, but the command itself can be tested without a button and generated from annotated screen-presenter methods.
\end{coruscobox}
...
\caption{The refresh companion example is deliberately plain: toolbar, small form, table, scroll pane, and status label. The point is to see the component tree and ordinary Swing responsibilities before adding Corusco structure.}
