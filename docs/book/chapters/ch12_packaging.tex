...
For user-facing examples and support output, the same discipline prevents embarrassing leaks. A figure reference, diagnostic entry, or generated report should use short stable names. It should not contain a long absolute path that happens to wrap badly because one developer's checkout was deep. The artifact should speak in product names, class names, feature ids, and generated state names.
...
The packaging figures show the release pipeline, installed layout, and support workflow as product states rather than as local build artifacts. They are deliberately plain because the important fact is traceability from source and generated code to the installed application.
\end{examplebox}
...
\begin{pitfallbox}{Dependency Output Pasted Without Review}
...
Example and support artifacts also have resources. Figures, sample data, generated screenshots, and diagnostic templates are usually not part of the production workflow, but they can still influence what users and reviewers see. Keep example-only helpers out of production launchers. Conversely, do not let user-facing support or help depend on resources that only exist in an IDE checkout. The build should make the boundary visible.

Companion examples should therefore be packaged as examples, not smuggled into a product runtime. They may need FlatLaf, MigLayout, deterministic data, and screenshot harnesses. Production applications may need FlatLaf and MigLayout too, but they do not need example-only generators unless the product intentionally exposes them. Separating these artifacts keeps the product lean.
...
Diagnostic artifacts follow the same rule. Example screenshots should come from deterministic states, generated sources should be produced by the same processors the application uses, and support bundles should record the runtime facts needed to reproduce a report. Users deserve evidence that can be rebuilt rather than screenshots or logs that only existed on one workstation.
...
Generated examples should have the same standard as hand-written examples. If an example demonstrates a generated form or table, it should compile through the processor used by the project. If the package relies on generated names, tests should fail when generation is stale. This protects examples from describing an API that the build no longer exercises.
...
The fifth drill is the example-artifact drill. Generate example screenshots and verify that cited examples exist. Delete a generated screenshot and confirm the build or review process notices before the artifact is trusted. Example artifacts should have the same intolerance for stale outputs as packaged applications.
...
The ninth drill is the stale-generated-artifact drill. Delete generated sources and generated screenshots, then run the release build from a clean checkout. The build should regenerate what it owns or fail with a clear diagnostic. If it silently succeeds because stale generated files were left elsewhere, the process is not clean enough for an application that relies on generation and screenshots.
...
\item Regenerate example screenshots from deterministic states before release.
\end{enumerate}
\end{exercisebox}
...
Example artifacts follow the same rule in miniature. A generated screenshot, a compact test log, and a clear warning scan do not make the example correct by themselves, but they prove that cited screens and generated companions are not imaginary. When the evidence is visible, stale screenshots and stale generated classes have fewer places to hide during review, audit, and release candidate work long after the build machine has moved on.
