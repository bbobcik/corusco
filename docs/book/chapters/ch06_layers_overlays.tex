...
\api{BookVisualFigures} contains the scope, busy, and validation overlay screenshots used in Figures~\ref{fig:layers-scope-coordinate}, \ref{fig:layers-busy-overlay}, and~\ref{fig:layers-validation-overlay}. The figures are generated by \api{BookFigureGenerator} through \screenshotHarnessExample{}, so the chapter's visual examples remain executable Swing surfaces.
\end{examplebox}
