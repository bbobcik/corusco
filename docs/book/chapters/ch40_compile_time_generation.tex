...
Primary annotations & \api{@CoruscoForm}, \api{@TextField}, \api{@DateField}, \api{@ComboBox}, \api{@CheckBox}, \api{@CoruscoTable}, \api{@Column}, \api{@UiAction}, validation annotations, and \api{@Help}.\\
...
ordinary Java source. A record annotated with \api{@CoruscoForm} describes an
editable value shape. A record annotated with \api{@CoruscoTable} describes a
...
\midrule
\api{@CoruscoForm} & Form prefix & Field ids, resource ids, problem codes, component contracts, descriptors\\
\api{@CoruscoTable} & Table prefix & Table id, default column ids, table-state boundary, descriptor ownership\\
...
Do not write runtime code that searches for \api{@CoruscoForm},
\api{@CoruscoTable}, or \api{@UiAction}. Runtime code should use generated
...
non-generic record annotated with \api{@CoruscoForm} and at least one field-kind
annotation on a record component. A table source is a non-generic record
annotated with \api{@CoruscoTable} and at least one \api{@Column}. A UI action
...
\api{@CoruscoForm} marks a record as a generated form source. Record components
...
model.name.setRawText("Grace Hopper", StandardChangeOrigin.USER);
...
\api{@CoruscoTable} marks a record as a generated table row source. Record
...
final class CustomerPresenter {
    ...
    void toggleActive() {
        activePreview.setValue(!activePreview.value(), StandardChangeOrigin.USER);
    }
}
\end{lstlisting}
...
Good generation fails early and locally. If \api{@CoruscoForm} is applied to a
...
Do not migrate only the annotations. A record with \api{@CoruscoForm} is useful
...
``add \api{@CoruscoForm}'' is not enough. Say that the source will generate a
...
The form source starts with \api{@CoruscoForm(id = "book/customer/edit")}. That
...
The table source starts with \api{@CoruscoTable(id = "book/customer/table")}.
...
\begin{exercisebox}
\begin{enumerate}
\item Annotate a small record with \api{@CoruscoForm}. Build the module and list
