...
\caption{A large-data table example rendered through the book screenshot harness. The table is visually ordinary on purpose: the interesting part is not decoration, but whether row identity, sorting, rendering, and update precision remain ordinary under load.}
...
\largeDataExample{} supplies the large table in Figure~\ref{fig:large-data-table}. \api{BookVisualFigures} supplies Figure~\ref{fig:table-coordinate-systems}, Figure~\ref{fig:table-refresh-identity}, and Figure~\ref{fig:tree-path-identity}. These figures are captured by \api{BookFigureGenerator} through \screenshotHarnessExample{} so the screenshots remain generated artifacts rather than detached illustrations.
\end{examplebox}
