...
Class and method names are printed in monospace and indexed where useful. The phrase \edt{} is used for the AWT/Swing event dispatch thread. "Model" means a Swing model such as \api{TableModel} when the surrounding context is Swing-specific; otherwise the text says "domain model", "application-scoped model", "presentation model", or "\corusco{} model" explicitly. "View" means the Swing component tree or the root object that owns it. "Presenter" is qualified when ambiguity matters: a screen presenter coordinates workflow and screen state, while a command presenter or command surface is a button, menu item, toolbar item, popup item, key binding, or accessibility action that exposes a command.
